...
en:{ dir:'ltr',
  ...
  removecolor:'Remove color',currentcolor:'Current color',customcolor:'Custom color…',
  ...
  tblalignleft:'Align table left',tblaligncenter:'Center table',tblalignright:'Align table right',
  alignnone:'None',
...
ar:{ dir:'rtl',
  ...
  removecolor:'إزالة اللون',currentcolor:'اللون الحالي',customcolor:'لون مخصص…',
  ...
  tblalignleft:'محاذاة الجدول لليسار',tblaligncenter:'توسيط الجدول',tblalignright:'محاذاة الجدول لليمين',
  alignnone:'بدون',
...
/* 25 swatches — a 5x5 grid in the picker, shared by fore/back color and cell fill */
const COLORS = [
  '#FFFFFF', '#F4F4F4', '#E8EAED', '#DADCE0', '#000000',

  '#F28B82', '#FBBC04', '#FFF475', '#CCFF90', '#A7FFEB',

  '#CBF0F8', '#AECBFA', '#D7AEFB', '#FDCFE8', '#E6C9A8',

  '#EA4335', '#FB8C00', '#FDD835', '#34A853', '#00ACC1',

  '#4285F4', '#5E35B1', '#8E24AA', '#C2185B', '#6D4C41'
];
...
function popup(anchor, build, cls){
  ...
  el.className = 'pop' + (cls ? ' '+cls : ''); el.dir = t('dir');
  build(el);
...
let tipEl = null, tipTarget = null, tipTimer = 0;
function hideTip(){
  clearTimeout(tipTimer); tipTarget = null;
...
function showTip(el){
  ...
  tipTimer = setTimeout(()=>{
    ...
    if(!tipEl){ tipEl = document.createElement('div'); tipEl.className='etip'; document.body.appendChild(tipEl); }
...
function buildPaletteInto(el, curColor, apply){
  ...
  cur.className='cswatch cur'; cur.title=t('currentcolor');
  cur.style.background=hex; cur.innerHTML=IC.check;
  const nocolor=document.createElement('button');
  nocolor.type='button'; nocolor.className='cswatch nocolor'; nocolor.title=t('removecolor');
  nocolor.addEventListener('click', ()=>{ closePop(); apply(null); });
  const wheel=document.createElement('label');
  wheel.className='cswatch cwheel'; wheel.title=t('customcolor'); wheel.innerHTML=IC.palette;
  const picker=document.createElement('input');
  picker.type='color'; picker.value=hex;
  picker.addEventListener('change', ()=>{ closePop(); apply(picker.value); });
...
function colorMenu(anchor, kind){
  popup(anchor, el=>buildPaletteInto(el, kind==='fore'?foreColor:backColor, c=>applyColor(kind,c)),
...
function buildCellFillInto(el){
  ...
  const cell=currentCell();
  buildPaletteInto(el, cssToHex(cell ? cell.style.backgroundColor : ''), apply);
}
function cellFillMenu(anchor){ popup(anchor, buildCellFillInto, 'pop-cellbg'); }
...
function buildTableCtxToolbar(){
  ...
  el.append(
    ...
    ctxSep(),
    ctxBtn(IC.alignleft, t('tblalignleft'), ()=>alignTable('left')),
    ctxBtn(IC.aligncenter, t('tblaligncenter'), ()=>alignTable('center')),
    ctxBtn(IC.alignright, t('tblalignright'), ()=>alignTable('right')),
    ctxSep(),
    fillBtn,
...
/* email-export direction detection - used only by getContentForEmail(). Content
   ...
   paragraph, but plain-text paste (normalizeTextPaste's dir="auto") and some
   other sources carry no reliable explicit direction at all. For those blocks
   we fall back to a first-strong-character scan (the same idea as HTML5's
   dir="auto") instead of the app's configured UI language, which has nothing
   to do with what the pasted text actually is. */
...
/* email-export table borders - used only by getContentForEmail(). Word describes
   a cell's borders as three parallel longhands (border-color / border-style /
   border-width), one value per side, and a browser re-copy of Word content keeps
   that split. Mail pipelines allowlist CSS one property at a time, and any of
   them that keeps border-color and border-width while dropping border-style
   erases the border outright - the initial border-style is none, so a width and
   a colour on their own draw nothing. Re-emitting every side as a single
   ...
   editor, and windowtext - a deprecated system colour Word still emits - becomes
   plain black. */
...
function sideBorderValue(side){
  ...
  const style = side.style===null ? 'solid' : side.style;
...
function hardenTableBorders(root){
  root.querySelectorAll('table,td,th').forEach(el=>{
    ...
    const rest=[];
    let sawBorder=false, bail=false;
    raw.split(';').forEach(decl=>{
      ...
      /* `border-color:inherit` and friends only mean anything against a parent
         this export has no say over - leave the cell untouched. */
      if(CSS_WIDE_KEYWORD.test(value)){ bail=true; return; }
      sawBorder=true;
      const one=m[1], part=m[2];
      if(!part){
        /* `border` / `border-<side>` resets all three components. */
        const parsed=parseBorderShorthand(value);
        (one?[one]:BORDER_SIDES.slice()).forEach(s=>{ sides[s]={...parsed}; });
      } else if(one){
        sides[one][part]=value;
      } else {
        const per=boxSides(splitCssValues(value));
        BORDER_SIDES.forEach((s,n)=>{ sides[s][part]=per[n]; });
      }
    });
...
window.getContentForEmail = function(){
  ...
  /* Borders last, so the per-side shorthands it writes are the final word on every
     cell. Also pin border-collapse inline and mirror it onto the legacy cellspacing
     attribute: a mail pipeline that drops border-collapse leaves the table in
     `separate` mode, where any cellspacing of its own would open gaps between
     Word's deliberately shared cell edges. */
